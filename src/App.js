...
function App() {
  ...
  const createUser = async (user,file) => {
    ...
    try {
      await getCreatedUser(user,file).then(res => {
        const result = res.data;
        MySwal.fire({
          icon: "success",
          title: "Product created successfully."
        }).then(() => {
          dispatch({ type: "CREATE_USER", data: result.data });
          setSavedUsers(oldvalues=>[...oldvalues, result.data]);
  ...
  const updateUser = async (id, updatedUser,image) => {
    setActiveModal(false);
    setLoading(true);
    try {
      await getUpdatedUser(id, updatedUser,image).then(res => {
        const result = res.data;
        MySwal.fire({
          icon: "success",
          ...
          dispatch({
            type: "SET_USERS",
            data: users.map(user =>
              user.id === id ? Object.assign(user, result.data) : user
            )
          });
        });
      });
